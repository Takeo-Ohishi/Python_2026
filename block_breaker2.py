...
IMG_PATH = os.path.join(BASE_DIR, "blocks/")
BLOCK_IMGs = ["block1.png", "block2.png", "block3.png", "block4.png", "block5.png"]
BALL_IMG = "Ball-a.png"

class Ball(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.image = pygame.image.load(IMG_PATH + BALL_IMG).convert_alpha()
...
class Block(pygame.sprite.Sprite):
    def __init__(self, x, y, image_path):
        super().__init__()
        self.image = pygame.image.load(IMG_PATH + image_path)
...
def main():
    ...
    for row in range(BLOCK_ROWS):
        for col in range(BLOCK_COLS):
            ...
            block = Block(block_x, block_y, BLOCK_IMGs[row % len(BLOCK_IMGs)])
            blocks.add(block)
